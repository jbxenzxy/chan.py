...
def _read_real_position(b):
    ...
    为什么必须把"可信"单独带出来（而不是各处写 `real_position(...) or 0`）：
      `real_position` 读数不可信时返回 `None`（未连接 / 行情停滞的通道不稳窗口 /
      读数异常 / 形态或取值认不出 —— 四个成因统一一个信号）。`None` 是 falsy，
      `... or 0` 会把它**抹成 0** —— 于是"开仓前读失败"与"平仓后读失败"两边都
      变成 0，`(0, 0) == (0, 0)` 恒成立：断言退化成空断言，冒烟照样打绿，
      实际一手指仓都没读到。
    这里统一按"两侧都必须是可信非负数"判定，把失败显式暴露给调用方。
    """
    lp = b.real_position(Side.LONG)
    sp = b.real_position(Side.SHORT)
    # `>= 0` 是**契约校验**不只是防御：`real_position` 承诺"非负 int 或 None"，
    # 冒烟脚本顺便验这条契约（实现若回流出数字哨兵，这里会判不可信而不是采纳）。
    ok = lp is not None and sp is not None and lp >= 0 and sp >= 0
    return ok, lp, sp


_READ_HINT = "（None=读数不可信：未连接/通道不稳/读数失败/形态或取值认不出）"
