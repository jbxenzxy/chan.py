...
1. `Docs/止盈止损/止盈止损四层策略原理.html:1334` 仍把已删的 `use_trailing` 当现役开关
   （`:1167` 的 `trailing_atr_multiple` 同理）；`:1160` 写「浮盈 ≥ …」并称
   「2026-09-14 起取代原全局 `trailing_trigger_r`」——浮盈判据现为**严格不等**，
   而 `trailing_trigger_r`（`Config.py:310`）仍在，只是角色已变。
2. `Docs/出场判定口径与1R播报_交付说明_20260922.html:249` 仍写「只对固定止盈单模式 / 旧库存量
