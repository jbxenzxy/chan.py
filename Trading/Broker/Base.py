...
class Broker(ABC):
    ...
    def real_position(self, side: "Side") -> Optional[int]:
        ...
        **返回契约（全模块唯一口径：实现方照此收敛，调用方照此判）**：
          · ``int >= 0`` —— 可信读数；0 表示**柜台该侧确实无仓**（可以采纳）；
          · ``None``      —— **不可信**，调用方必须跳过该侧，不得当成 0 采纳。
        "不支持对账"（dry_run 等）与"读数失败/读数不可信"**共用 None**：两者对
        调用方的含义相同（没有可用读数），故不再引入第二个哨兵值。

        为什么不用负数表达"读不到"：负数是个**合法整数**，漏挡的调用方会把它
        当手数算下去（`engine_vol - (-1)` 比账本多平一手、整侧仓单被删并补记
        虚构盈亏）；而 None 漏挡会直接 `TypeError` 暴露，不会静默算错手数。
