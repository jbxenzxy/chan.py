...
为什么有本文件
--------------
交接文档 §3.4 批判过一种反模式：**同一份数据存两份 + 配一套对账**（当时批的是
price_tick 与行情的那 122 行）。而 P-A 落地费率时又重建了同一结构：xlsx 一份、
...
**为什么是"标记区块"而不是独立文件**（2026-09-15 用户拍板）
------------------------------------------------------------
最初实现为 `Trading/Infra/_FeeTable.py`（独立生成文件）。用户指出这与
交接文档 §5.1 冲突 —— 那一节把 `Trading/Infra/` 定死成 **7 个模块、按变异轴排**
