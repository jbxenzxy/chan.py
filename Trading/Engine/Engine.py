...
每根 K 线的处理顺序（顺序错了结果就错了）
    ① 结算已有持仓 —— 用**刚闭合**的 K 线的 close 判止盈止损 L1-L3
    ② 才处理落在这根 K 线上的信号 —— 决定开仓
    反过来会变成"同一根 K 线内既开仓又平仓"，是回测里最常见的作弊来源。

两处时序防护（踩过才知道）
    ① 入场那根 K 线不能参与出场判定：信号在 K 线 T 闭合时产生、按 T 的收盘价开仓，
       而判定依据现在也是收盘价 —— 在 T 上判等于拿开仓价自己去比止损/止盈线，
       没有信息量；更麻烦的是 `_run_plan` 可能是 state.db 原样恢复的计划（不重跑
       plan() 里的 P2 边界守卫），止损线若落在入场价内侧就会瞬间误触发。
...
class TradingEngine(ReconcileMixin):
    ...
    def _settle_positions(self, bar: Bar) -> None:
        ...
        # 而判定依据现在也是收盘价 —— 在 T 上判等于拿开仓价自己去比止损/止盈线；
        # 且 `_run_plan` 可能是 state.db 原样恢复的计划（不重跑 plan() 的 P2 边界守卫），
        # 止损线若落在入场价内侧就会瞬间误触发。
