...
├── tools/                  # 辅助工具（录制 / 生成 / 运维 / 探针）
│   ├── make_demo_data.py   # 生成 demo 数据
│   ├── gen_replay_data.py  # 生成贴近市价的回放数据（M2b 回归用）
│   ├── m0_recorder/        # M0 信号录制器（上游数据源，零侵入订阅 chan.py SSE）
│   │   ├── signal_recorder.py  # 录制买卖点信号完整生命周期 + K线
│   │   └── analyze.py          # 重绘率统计 + 止盈止损参数回测
│   └── simnow/             # SimNow 运维 / 探针工具（均需 tqsdk）
│       ├── simnow_diag.py       # 账户资金 / 持仓 / 委托诊断
│       ├── simnow_force_close.py# 独立强制平仓（清残留仓，限价让价 + 重试）
│       ├── probe_after_close.py # 收盘后可交易性探测
│       ├── simnow_probe.py      # M2a 通道探针（登录 / 行情 / 下单四步自检）
│       ├── simnow_smoke_trade.py# M2b 真成交冒烟（贴近市价下 1 手闭环）
│       └── README_m2a_probe.md  # M2a 探针说明
└── tg/
...
M0 录制器（`tools/m0_recorder/`）产出的 `signals.json` / `klines.json` 可直接作为回放输入，无需任何转换。拿到 ≥5 个交易日真实信号后：
...
> ⚠️ 首次接 simnow 前，先跑通道探针（`tools/simnow/simnow_probe.py`）确认登录 / 行情 / 下单通道，再上 simnow broker；避免账号坑全链路排查。跑之前 / 之后随时可用 `tools/simnow/simnow_diag.py` 查账户真实状态（资金 / 持仓 / 未成交委托）。
...
## 九、辅助工具（trader_gateway/tools/）

这些脚本**不进 `tg/` 核心引擎**，是围绕网关的录制 / 生成 / 运维 / 探针工具。除 `m0_recorder`（只依赖标准库）外，所有 `simnow/` 下工具都需要 `pip install tqsdk`，且凭据从环境变量读取（与网关一致）。

### 9.1 M0 信号录制器 `tools/m0_recorder/`（上游数据源，零侵入）
| 脚本 | 作用 |
|---|---|
| `signal_recorder.py` | 订阅 chan.py 的 SSE 端点，录制买卖点信号完整生命周期（出现/位移/消失）+ K线，落盘 `signals.json`/`klines.json` |
| `analyze.py` | 读录制数据，统计重绘率、信号振幅分布、止盈止损参数回测 |

```bash
python tools/m0_recorder/signal_recorder.py --symbol "KQ.m@CFFEX.IF" --freq 5m --out ./out_if5m
python tools/m0_recorder/analyze.py --out ./out_if5m --scan
```

### 9.2 SimNow 运维 / 探针 `tools/simnow/`（需 tqsdk）
| 脚本 | 作用 | 何时用 |
|---|---|---|
| `simnow_probe.py` | 通道四步自检：登录 → 资金 → 主连映射 → 测试下单 | **首次接 simnow 前** |
| `simnow_diag.py` | 查账户资金 / 持仓 / 未成交委托；`--close-all` 市价平掉所有持仓 | 跑网关前 / 后查状态 |
| `simnow_force_close.py` | 独立强制平仓（限价让价 + 重试，绕开中金所市价单限制） | 清残留仓、幽灵仓 |
| `probe_after_close.py` | 探测收盘后 SimNow 是否可登录 / 报单是否被拒 | 非交易时段想跑回归时 |
| `simnow_smoke_trade.py` | 贴近市价下 1 手，验证成交→持仓→平仓闭环 | 快速冒烟（不跑整个网关） |

> 注：`simnow_probe.py` / `simnow_smoke_trade.py` 是 M2a/M2b 阶段的早期通道验证脚本，其能力已被 `tg/brokers/simnow.py`（`--broker simnow`）完全覆盖。日常请直接用 `run_gateway.py`；这两个脚本仅作为**不依赖整个引擎的轻量一键验证**保留，如想极致精简可直接删除。

凭据（与网关一致，从环境变量读，不硬编码）：
```bash
set SN_ACCOUNT=... && set SN_PASSWORD=... && set TQ_ACCOUNT=... && set TQ_PASSWORD=...
```
