...
class ChartHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        """[墓碑 · 阶段 3a] 遗留 http.server 入口已退役（410 Gone）

        原 774 行 REST/SSE 分发逻辑已于阶段 3a 迁入 FrontAPI.py
        （单一路由源，经 App/AppOrch.py 编排，引擎调用带锁分类）。
        请改用 FastAPI 入口：

            python FrontAPI.py            # 统一入口（推荐）
            python api_server.py          # 兼容入口（同一 app 实例）
            uvicorn FrontAPI:app

        SSE 实时方法（_handle_sse_stream_dual / _handle_sse_stream_single）
        按设计 8.6-3b 保留至 3b-2（灰度通过后拆除），由 FrontAPI 的
        legacy 桥接层（impl=legacy）经 AppOrch.get_sse_handler 调用，
        不经本方法分发。
        """
        self.send_json_response({
            "error": "Gone",
            "detail": "本 HTTP 入口已于阶段 3a 退役，请使用 FastAPI 入口："
                      "python FrontAPI.py 或 uvicorn FrontAPI:app",
            "tombstone": "my_chan_main.ChartHandler.do_GET",
            "phase": "3a",
            "successor": "FrontAPI.py",
        }, 410)
    ...
    def _handle_sse_stream_single(self, symbol, freq="15s", start_time=None):
        ...
        chan = None
        # 前置处理：别名解析 + 频率映射 + 显示键。
        # 纯参数处理（不依赖网络/引擎），置于 try 之外，display_key/freq_sec
        # 在函数入口即确定，except/finally 引用为正常作用域。
        symbol_upper = symbol.upper()
        if symbol_upper in CTqSdkAPI.FUTURES_ALIASES:
            symbol = CTqSdkAPI.FUTURES_ALIASES[symbol_upper]
        freq_sec = CTqSdkAPI.FREQ_SEC_MAP.get(freq, 15)
    ...
    def do_POST(self):
        """[墓碑 · 阶段 3a] 遗留 http.server POST 入口已退役（410 Gone）

        原标注增删改 / 盘后下载启动逻辑已迁入 FrontAPI.py
        （/api/annotations POST、/api/tdx_download_start POST）。
        """
        self.send_json_response({
            "error": "Gone",
            "detail": "本 HTTP 入口已于阶段 3a 退役，请使用 FastAPI 入口："
                      "python FrontAPI.py 或 uvicorn FrontAPI:app",
            "tombstone": "my_chan_main.ChartHandler.do_POST",
            "phase": "3a",
            "successor": "FrontAPI.py",
        }, 410)
...
def main():
    ...
    # 2. 启动HTTP服务器（[墓碑 · 阶段 3a] 本入口仅回 410，保留仅为兼容进程脚本）
    # do_GET/do_POST 已墓碑化：所有请求返回 410 Gone + 迁移指引。
    # 前端/API 请使用 FastAPI 入口：python FrontAPI.py（或 uvicorn FrontAPI:app）
    print("[墓碑 · 阶段 3a] 本 HTTP 入口已退役（全部请求将返回 410 Gone）")
    print("[信息] 请改用 FastAPI 入口: python FrontAPI.py（或 python api_server.py 兼容入口）")
    port = 18081
    ...
    print(f"[信息] 墓碑服务器已启动: {url}（SSE 方法保留至 3b-2 供 legacy 桥接调用）")
