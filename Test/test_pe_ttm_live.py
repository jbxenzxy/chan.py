...
from App.AppEngine import _fetch_pe_ttm_live                 # noqa: E402
...
def test_source_routing():
    ...
    def fake_eltdx_single(market, code):
        seen.setdefault("eltdx_single", []).append((market, code))
        return {market + code: 10.0}

    def fake_tx(pairs):
        seen.setdefault("tx", []).extend(list(pairs))
        return {m + c: 11.0 for m, c in pairs}

    import App.AppEngine as R
    # 注意：PE-TTM 选源编排 `_fetch_pe_ttm_live` 现收口在 AppEngine（装配点，
    # 2026-09 由 AppRefresh 迁入）。AppEngine 是
    # `from DataAPI.TxAPI import fetch_pe_ttm as _tx_fetch_pe_ttm` 的直接名绑定，
    # 打桩必须打在 **AppEngine 侧的名字** 上；只改 TxAPI.fetch_pe_ttm
    # 不生效 —— 那样用例会真的联网请求腾讯（既非"不联网"，结果也不稳定）。
    _e, _t = R._eltdx_fetch_pe_ttm_single, R._tx_fetch_pe_ttm
    R._eltdx_fetch_pe_ttm_single, R._tx_fetch_pe_ttm = fake_eltdx_single, fake_tx
    try:
        r_a = R._fetch_pe_ttm_live("sh", "600519")      # A股个股 → eltdx 单只
        r_idx = R._fetch_pe_ttm_live("sh", "000001")    # 上证指数 → 腾讯
        r_hk = R._fetch_pe_ttm_live("hk", "00700")      # 港股 → 腾讯
        ok = (seen.get("eltdx_single") == [("sh", "600519")]
              and seen.get("tx") == [("sh", "000001"), ("hk", "00700")]
              and r_a == {"sh600519": 10.0}
              and r_idx == {"sh000001": 11.0} and r_hk == {"hk00700": 11.0})
        rec("⑧", "分流：A股个股→eltdx单只 / 指数·港股→腾讯", ok,
            f"eltdx 单只收到={seen.get('eltdx_single')}；腾讯收到={seen.get('tx')}；"
            f"返回={r_a}/{r_idx}/{r_hk}")
    finally:
        R._eltdx_fetch_pe_ttm_single, R._tx_fetch_pe_ttm = _e, _t
