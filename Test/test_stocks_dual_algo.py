...
=====================================================================
改造报告 P0 要求：抽出 _stocks_sub_dt_algo（下窗截断边界）与
...
设计要点（与期货公式镜像对称）：
  ...
  · 日期型主级别（w/d）：左界 = 覆盖期首日 00:00，右界 = 右边界当日 23:59:59
...
def test_red_range_algo_intraday(failures):
    ...
    例：A=11:00（左边界 30m 线结束时刻，覆盖 (10:30,11:00]），B=11:30。
        offset = 30m - 5m = 25m。
...
def test_red_range_algo_daily(failures):
    ...
    例：A=2024/01/02（左边界日线），B=2024/01/05（右边界日线）。
