...
用法：
    python App/ths_capture_cookie.py
    （阶段 2 起本工具位于 App/，Cookie 文件 App/ths_captured_cookie.txt 随之自动跟随）
...
def manual_input_cookie() -> str:
    """引导用户手动粘贴 Cookie"""
    log.info("\n" + "=" * 60)
    log.info("请手动获取 Cookie：")
    log.info("  1. 浏览器打开 https://t.10jqka.com.cn 并登录同花顺")
    log.info("  2. F12 → Network → 刷新页面")
    log.info("  3. 点击任意请求 → Request Headers → 复制 Cookie 值")
    log.info("  4. 在下面粘贴 Cookie（回车两次结束输入）：")
    log.info("=" * 60)
...
def main():
    ...
    if not cookie:
        log.info("[Cookie] 未输入 Cookie，退出")
        return

    log.info("[Cookie] 正在验证手动输入的 Cookie...")
    if check_cookie_valid(cookie):
        log.info("[Cookie] ✅ Cookie 验证通过")
        save_cookie(cookie)
    else:
        log.info("[Cookie] ❌ Cookie 验证失败，请确认：")
        log.info("        - 是否已登录同花顺 https://t.10jqka.com.cn")
        log.info("        - Cookie 是否复制完整（不要遗漏分号）")
