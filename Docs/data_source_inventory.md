...
| 重要股东买卖（股东增减持计划） | `DataAPI/ElTdxAPI.py` | `get_shareholder_reduction_flag(market, code, today)` / `get_shareholder_reduction_plans(market, code)`（ElTdxAPI 末尾新增，约 `:570`） | **eltdx F10 7615 网关**：`F10Client.shareholder_change_plans(code)` → `CWServ.tdxf10_gg_gdyj` + section `gdzjcjh` · **按代码查询**（非全市场文件）· 进程缓存 1 天、不落盘 · 仅覆盖 sh/sz/bj · K 线页打开标的时经 `AppEngine._get_reduction_flag` 调用，命中「拟减持」且当前交易日在 `公告日(N001)~截止日(N010)` 窗口内 → 前端 PE-TTM 后显示 `减持：√` | `AppEngine.py:1241` `meta.shareholder_reduction` → `Frontend/app.js:2268` 徽标 |
