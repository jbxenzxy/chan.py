...
class ChartHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/stock":
            ...
            # 销毁该周期的缓存
            cache_key = f"{_CACHE_VERSION}_{market}_{normalized_code}_{freq}" if market else f"{_CACHE_VERSION}_{normalized_code}_{freq}"
            if cache_key in _analysis_cache:
                del _analysis_cache[cache_key]
            import gc
            gc.collect()
        ...
        function drawViewportHighLow(klines, area, priceRange, barStep, subPixelOffset) {
            ...
            function drawOne(price, klineIdx, isHigh) {
                ...
                if (canLeft) {
                    ...
                    ctx.fillText(text, textX, ky);
                    ctx.fillText(arrowR, textX + textW + gap, ky);
                } else {
                    // ← 数值
                    const arrowX = kx;
                    const textX = arrowX + arrowW + gap;
                    if (textX + textW > area.x + area.w) return;
                    ctx.fillText(arrowL, arrowX, ky);
                    ctx.fillText(text, textX, ky);
                }
            }
